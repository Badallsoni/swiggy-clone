...
<div className="flex w-full justify-between mb-2 pb-2">
  <div className="w-[70%]">
    <p className="text-gray-700 font-semibold text-xl">{restData?.name}</p>

    <p>{"₹" +("defaultPrice" in restData
  ? restData?.defaultPrice / 100
  : restData?.price / 100)}</p>
  ...
  <div className="w-[20%] relative">
    ...
    { count===0?
   ( <button className="absolute bottom-1 left-20 text-green-600 rounded-2xl px-4 py-2 bg-white" onClick={() =>  handleAdditems()}>
  ADD
</button>):(<div className="flex gap-2 text-2xl text-green-600 px-6 py-2 shadow-md border border-white">
